...
$file  = Get-Content -AsByteStream $args[0]
...
Set-Content -AsByteStream -Path ".\signature.p7" -Value $sigbytes
Write-Host "Successfully created output file .\signature.p7" -ForegroundColor green

# Build and write variable content output file
$content = $chop[$sig_length..($chop.Length - 1)]
[System.Byte[]] $bytes =  @()
foreach ($i in $content) {$bytes += $i}
Set-Content -AsByteStream -Path ".\content.bin" -Value $bytes
Write-Host "Successfully created output file .\content.bin" -ForegroundColor Green
